//! 全拼音节表全覆盖：词库里在册的每个读音，segment() 都必须能作为单音节切出。
//!
//! 期望集来源：线上词库 `~/.local/share/kime/dict.sqlite3` phrase.pinyin 的
//! 全部去重音节（416），逐条核对均为 rime-luna-pinyin.dict.yaml 音节集（424）
//! 的成员（https://github.com/rime/rime-luna-pinyin ）。词库有读音而音节表缺
//! → 该词全拼打不到 —— 用户报「双拼不全」，全拼侧同样的病在这里治。

use kime_pinyin::segment;

/// 416 = 原 401 标准音节 + 15 个词库在册的罕见音节
/// （biang cei den dia fiao kei lo nia nou nun rua tei ya yo zuan）。
...
#[test]
fn every_syllable_segments_as_itself() {
    assert_eq!(
        ALL_SYLLABLES.len(),
        416,
        "期望集规模须与 src 音节表同步（table_sanity 钉住另一侧）"
    );
    for &s in ALL_SYLLABLES {
        let r = segment(s);
        assert!(
            r.iter()
                .any(|reading| reading.len() == 1 && reading[0] == s),
            "segment({s:?}) 切不出自身: {r:?}"
        );
    }
}
...
#[test]
fn rare_readings_recovered() {
    // 补录前切不出自身、或只能错误拆碎的在册读音
    assert_eq!(segment("ya"), vec![vec!["ya".to_string()]]);
    assert_eq!(segment("yo"), vec![vec!["yo".to_string()]]);
    assert_eq!(segment("kei"), vec![vec!["kei".to_string()]]);
}
